...
@echo off
if /I %arch%==AMD64 (
    start /wait %curdir%\installers\WPTx64-x86_en-us.msi
    set copyflag=1
) else if /I %arch%==X86 (
    start /wait %curdir%\installers\WPTx86-x86_en-us.msi
    set copyflag=1
) else (
    start /wait %curdir%\installers\WPTarm-arm_en-us.msi
    set copyflag=0
)
...
::
copy /y %curdir%\bin\%arch%\*.dll "%wpapath%"
